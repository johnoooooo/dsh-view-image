...
function imageMarkerText(ref) {
	...
	return `[图片附件: ${ref.attachmentId}]（${ref.mediaType}, ${ref.width}x${ref.height}, ${ref.bytes} bytes）. Use the view_image tool to identify this image: ${parts.join(", ")}.`;
}
...
async function rewriteImageParts(ctx, content) {
	...
	for (const part of content) {
		...
		if (data.byteLength === 0) {
			blocks.push({ type: "text", text: "[图片附件: empty data]" });
			continue;
		}
				...
				if (attachmentId.length > 0) {
					...
					try {
						...
						if (error?.code === "ATTACHMENT_NOT_FOUND" || error?.code === "ATTACHMENT_CORRUPT") {
							throw new Error(`view_image: failed to read image attachment ${attachmentId} (${error.code}): attachment missing, corrupted, or marker fields do not match storage. Re-paste the image, or pass media_type / attachment_width / attachment_height / attachment_bytes from the [图片附件] marker verbatim.`);
						}
						throw error;
					}
					bytes = stored?.data;
