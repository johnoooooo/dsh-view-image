...
/**
 * 判断当前 session 路由是否为纯文本（模型不支持 image 输入）。
 * 返回 true 表示「需要把图片改写成标记」；false 表示模型自己能看图，无需干预。
 */
async function routeNeedsImageRewrite(ctx) {
	const llm = ctx.get("llm");
	const agentDefault = ctx.get("agentDefaultModel");
	if (!llm || !agentDefault) return false;
...
function wrapPromptHandler(ctx, logger) {
	...
	api.sessions.prompt = async function (request, signal) {
		try {
			const payload = request?.payload;
			const content = payload?.content;
			if (Array.isArray(content) && content.some((p) => p?.type === "image")) {
				if (await routeNeedsImageRewrite(ctx)) {
	...
	ctx.on("dispose", () => {
		if (api.sessions.__viewImageWrapped) api.sessions.prompt = original;
	});
}
...
function apply(ctx, config) {
	...
	ctx.systemPrompt.section({
		...
		text: [
			...
			"The current model does not accept image input, so read_image will fail — always prefer view_image for images.",
		].join(" "),
	});
	...
	ctx.tools.register(
		defineTool({
			...
			parameters: {
				...
				attachment_width: {
					type: "number",
					description: "Image width px, from the [图片附件] marker. Required with attachment_id.",
				},
				attachment_height: {
					type: "number",
					description: "Image height px, from the [图片附件] marker. Required with attachment_id.",
				},
				attachment_bytes: {
					type: "number",
					description: "Encoded image byte count, from the [图片附件] marker. Required with attachment_id.",
				},
				intent: {
					type: "string",
			...
			async execute(args, exec) {
				...
				if (attachmentId.length > 0) {
					// ── 按附件 id 读取持久化图片字节（ref 字段来自 [图片附件] 标记）──
					const attachments = ctx.get("attachments");
					...
					bytes = stored?.data;
					mediaType = stored?.mediaType ?? declaredMediaType;
