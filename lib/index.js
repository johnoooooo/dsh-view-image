...
/** 默认 intent 软引导文案：模型生成 intent 时参考；用户明确要求时优先用用户的。 */
const INTENT_GUIDANCE_DEFAULT =
	'If the user gives no specific focus, describe the image in full by default: 完整描述图片中所有内容，包括所有文字、图形和布局结构，用中文回答。';
...
const Config = z.object({
	...
	/** 软引导：主模型生成 intent 时参考的默认偏好；用户明确给出关注点时仍以用户为准。空字符串禁用引导。 */
	intentGuidance: z.string().default(INTENT_GUIDANCE_DEFAULT),
...
function apply(ctx, config) {
	...
	const cfg = {
		...
		intentGuidance: config?.intentGuidance ?? INTENT_GUIDANCE_DEFAULT,
	...
	ctx.tools.register(
		defineTool({
			...
			parameters: {
				...
				intent: {
					type: "string",
					description: "What to focus on, e.g. \"提取图中文字\" or \"描述图表内容\". " + (cfg.intentGuidance ? cfg.intentGuidance : ""),
				},
			},
			output: {
				schema: {
