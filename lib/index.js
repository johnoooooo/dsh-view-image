...
const DEFAULT_PROMPT =
	"Describe the image in full, including all text (extract verbatim, do not translate), figures, and layout.";

/** 默认 intent 后缀：拼接在 intent 内容之后发给视觉模型（可配置覆盖）。 */
const INTENT_SUFFIX_DEFAULT = "Analyze the image according to the requirements above.";
...
function defaultIntentGuidance(prompt) {
	return `If the user gives no specific focus, describe the image in full by default, in the same language as the user's message: ${prompt}`;
}
...
const Config = z.object({
	...
	/** 模型未传 intent 时的默认 intent 内容（拼后缀后发给视觉模型）。 */
	defaultPrompt: z.string().default(DEFAULT_PROMPT),
	/** intent 内容之后拼接的后缀（默认英文；空字符串禁用后缀）。 */
	intentSuffix: z.string().default(INTENT_SUFFIX_DEFAULT),
...
function imageMarkerText(ref) {
	...
	return `[Image Attachment: ${ref.attachmentId}] (${ref.mediaType}, ${ref.width}x${ref.height}, ${ref.bytes} bytes). Use the view_image tool to identify this image: ${parts.join(", ")}.`;
}
...
async function requestVisionModel(vision, prompt, dataUrl, mediaType, signal, timeoutMs) {
	...
	try {
		...
		if (timedOut)
			throw new Error(`view_image: vision model request timed out (${timeoutMs}ms)`);
		if (signal.aborted) throw new Error("view_image: request was cancelled");
		throw new Error(`view_image: vision model request failed: ${error instanceof Error ? error.message : String(error)}`);
	}
	...
	if (!resp.ok) {
		...
		throw new Error(`view_image: vision model returned HTTP ${resp.status}${detail ? `: ${detail}` : ""}`);
	}

	const data = await resp.json().catch(() => null);
	if (!data) throw new Error("view_image: vision model returned an unparseable response");
	...
	if (!text) {
		...
		throw new Error(`view_image: vision model returned no valid text (finish_reason=${stop})`);
	}
	return text;
}
...
async function rewriteImageParts(ctx, content) {
	...
	for (const part of content) {
		...
		if (data.byteLength === 0) {
			blocks.push({ type: "text", text: "[Image Attachment: empty data]" });
			continue;
		}
		...
		try {
			...
			if (Array.isArray(content) && content.some((p) => p?.type === "image")) {
				if (await routeNeedsImageRewrite(ctx, String(payload?.sessionId ?? ""))) {
					...
					logger.info(`text-only route: rewrote pasted images into attachment markers (${rewritten.filter((b) => b.type === "text" && b.text.includes("[Image Attachment")).length} image(s))`);
					return original(copy, signal);
				}
			}
		} catch (error) {
			logger.warn(`image rewrite failed, falling back to the native flow: ${error instanceof Error ? error.message : String(error)}`);
		}
...
function apply(ctx, config) {
	...
	const cfg = {
		...
		intentSuffix: config?.intentSuffix ?? INTENT_SUFFIX_DEFAULT,
	...
	if (!vision) {
		...
		logger.warn(`view_image not loaded: missing ${visionPath} (baseUrl / apiKey / model required). See vision-model.example.opencode.json.`);
		return;
	}
	...
	ctx.systemPrompt.section({
		...
		text: [
			...
			"Use view_image when: (1) the user mentions an image file path (.png/.jpg/.jpeg/.webp/.gif/.bmp), or (2) a pasted image arrives as a text marker like [Image Attachment: <attachmentId>] — call view_image with attachment_id=\"<attachmentId>\".",
	...
	ctx.tools.register(
		defineTool({
			...
			description:
				...
				"Optionally pass intent to focus extraction (e.g. \"extract the text\" or \"describe the chart\").",
			parameters: {
				file_path: {
					type: "string",
				...
				attachment_id: {
					type: "string",
					description: "Durable attachment id of a pasted image (the [Image Attachment: <id>] marker's id).",
				},
				media_type: {
					type: "string",
					description: "Media type of the attachment image (image/png, image/jpeg, ...), from the [Image Attachment] marker.",
				},
				attachment_width: {
					type: "number",
					description: "Image width px, from the [Image Attachment] marker. Pass when known; verified against stored metadata.",
				},
				attachment_height: {
					type: "number",
					description: "Image height px, from the [Image Attachment] marker. Pass when known; verified against stored metadata.",
				},
				attachment_bytes: {
					type: "number",
					description: "Encoded image byte count, from the [Image Attachment] marker. Pass when known; verified against stored metadata.",
				},
				intent: {
					type: "string",
					description: "What to focus on, e.g. \"extract the text\" or \"describe the chart\". " + (intentGuidance ? intentGuidance : ""),
				},
			},
			output: {
				schema: {
			...
			async execute(args, exec) {
				...
				if (attachmentId.length > 0) {
					...
					try {
						...
						if (error?.code === "ATTACHMENT_NOT_FOUND" || error?.code === "ATTACHMENT_CORRUPT") {
							throw new Error(`view_image: failed to read image attachment ${attachmentId} (${error.code}): attachment missing, corrupted, or marker fields do not match storage. Re-paste the image, or pass media_type / attachment_width / attachment_height / attachment_bytes from the [Image Attachment] marker verbatim.`);
						}
						throw error;
					}
					bytes = stored?.data;
				...
				// intent 恒有值：模型传了用模型的，没传用默认值（defaultPrompt 内容）。
				// 视觉模型提示词统一为「intent + 后缀」。
				const intent = String(args.intent ?? "").trim();
				const effectiveIntent = intent.length > 0 ? intent : (cfg.defaultPrompt ?? DEFAULT_PROMPT);
				const prompt = cfg.intentSuffix ? `${effectiveIntent}\n\n${cfg.intentSuffix}` : effectiveIntent;
